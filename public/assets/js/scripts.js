...
(function ($, window) {
	...
	$.fn.navigationResize = function () {
		...
		var $navItemMore = $menuContainer.find('li.garantili-flex-more-menu-item');
		...
		var $navItems = $navItemMore.parent().children('li:not(.garantili-flex-more-menu-item)'),
			navItemMoreWidth = $navItemMore.outerWidth() + 20,
...
(function ($) {
	...
	// 	$.blockUI.defaults.overlayCSS.background        = '#fff url(' + garantili_options.ajax_loader_url + ') no-repeat center';
	...
	$('body').on('woocommerce_variation_has_changed', function (e) {
		...
		if (typeof $availability !== "undefined" && $availability !== false) {
			$('.garantili-stock-availability').html($availability);
		}
	});
	...
	$(".section-categories-filter").each(function () {
		...
		$this.find('.categories-dropdown').change(function () {
			...
			$.ajax({
				url: garantili_options.ajax_url,
				type: 'post',
				data: {
	...
	$(document).ready(function () {
		...
		$('.site-header .garantili-sticky-wrap').each(function () {
		...
		// if( garantili_options.enable_live_search == '1' ) {

		// 	if ( garantili_options.ajax_url.indexOf( '?' ) > 1 ) {
		// 		var prefetch_url    = garantili_options.ajax_url + '&action=products_live_search&fn=get_ajax_search';
		// 		var remote_url      = garantili_options.ajax_url + '&action=products_live_search&fn=get_ajax_search&terms=%QUERY';
		// 	} else {
		// 		var prefetch_url    = garantili_options.ajax_url + '?action=products_live_search&fn=get_ajax_search';
		// 		var remote_url      = garantili_options.ajax_url + '?action=products_live_search&fn=get_ajax_search&terms=%QUERY';
		// 	}
		...
		// 	$( '.navbar-search .product-search-field' ).typeahead( garantili_options.typeahead_options,
		// 		{
		// 			name: 'search',
		// 			source: searchProducts.ttAdapter(),
		// 			displayKey: 'value',
		// 			limit: garantili_options.live_search_limit,
		// 			templates: {
		// 				empty : [
		// 					'<div class="empty-message">',
		// 					garantili_options.live_search_empty_msg,
		// 					'</div>'
		// 				].join('\n'),
		// 				suggestion: Handlebars.compile( garantili_options.live_search_template )
		// 			}
		// 		}
		// 	);
		// }
